...
export default function DeveloperProfilePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        // Get user
        const uSnap = await getDocs(query(collection(db,"users"), where("__name__","==",devId)));
        if (!uSnap.empty) setDev({ id: uSnap.docs[0].id, ...uSnap.docs[0].data() } as Developer);

        // Get their models
        const mSnap = await getDocs(query(collection(db,"models"), where("authorId","==",devId), where("status","==","published"), orderBy("uploadedAt","desc"), limit(12)));
              ...
              <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.5, delay:0.05 }}>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-black text-white">
                    Published <span style={{ backgroundImage:"linear-gradient(90deg,#a78bfa,#22d3ee)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>Models</span>
                  </h2>
