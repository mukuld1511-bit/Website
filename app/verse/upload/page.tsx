...
export default function VerseUploadPage() {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setTimeout(() => { window.location.href = "/verse"; }, 2000);
  ...
  return (
    <div className="min-h-screen bg-[#F7F6F3] flex flex-col font-sans">      <div className="max-w-2xl mx-auto px-4 py-14 flex-grow w-full">
        ...
        {/* Back + header */}
        <div className="mb-8">
          <Link href="/verse" className="inline-flex items-center gap-2 text-gray-400 hover:text-gray-700 text-sm font-semibold mb-5 transition">
        ...
        {step === 2 && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div>
              ...
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Low Poly Industrial Warehouse" className={inputCls} />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Description</label>
              <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3}
                placeholder="Describe your model..." className={inputCls + " resize-none"} />
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-2">
                {tags.map(t => (
                  <button key={t} onClick={() => setTags(tags.filter(x => x !== t))}
                    className="px-2.5 py-1 rounded-lg text-xs font-bold bg-violet-50 text-violet-700 border border-violet-200 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition">
                    {t} ✕
                  </button>
                ))}
              </div>
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="pt-3">
                  ...
                  <div className="relative">
                    ...
                    <input type="number" value={price || ""} onChange={e => setPrice(parseFloat(e.target.value) || 0)}
                      placeholder="299" min="0" className={inputCls + " pl-8"} />
                  </div>
            ...
            {uploading && (
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-semibold">
                  <span className="text-violet-600">{uploadStage}</span>
                  <span className="text-gray-500">{uploadProgress}%</span>
                </div>
                <div className="w-full h-2.5 rounded-full bg-gray-100 overflow-hidden">
                  <motion.div animate={{ width: `${uploadProgress}%` }} className="h-full bg-gray-900 rounded-full" />
                </div>
              </div>
            )}
