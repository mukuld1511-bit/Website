      ...
      <div className="p-4 space-y-3">
        ...
        <div className="bg-gray-50 rounded-xl px-3 py-2.5 text-xs text-gray-500 space-y-0.5">
          <div className="flex justify-between">
            ...
            <span className="font-medium text-gray-700 truncate max-w-[60%]">{modelMeta.fileName}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-400">Type</span>
            <span className="font-medium text-gray-700">{modelMeta.fileType}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-400">Category</span>
            <span className="font-medium text-gray-700">{modelMeta.category}</span>
          </div>
        </div>
            ...
            {loading ? (
              <>
                <div className="flex gap-0.5">
                  {[0,1,2].map(i => (
                    <div key={i} className="w-1 h-1 rounded-full bg-white animate-bounce" style={{ animationDelay: `${i * 0.12}s` }} />
                  ))}
                </div>
                Generating…
              </>
            ...
            >
              ...
              <div className="bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5">
                ...
                <div className="flex flex-wrap gap-1">
                  {result.suggestedTags.map(tag => (
                    <span key={tag} className="text-xs bg-[#5B4BDB]/10 text-[#5B4BDB] px-2 py-0.5 rounded-full font-medium">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5 flex justify-between items-center">
                ...
                <p className="text-sm font-bold text-amber-900">
                  {result.suggestedPrice === 0 ? "Free" : `₹${result.suggestedPrice}`}
                </p>
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  onClick={generate}
                  className="flex-1 py-2 border border-gray-200 text-xs font-semibold text-gray-600 rounded-xl hover:bg-gray-50 transition-all"
                >
                  Regenerate
                </button>
                <button
