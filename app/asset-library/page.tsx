...
export default function AssetLibrary() {
  ...
  const inputCls = "w-full bg-white border-2 border-indigo-100 text-white placeholder-gray-400 rounded-2xl px-4 py-3.5 focus:outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100 transition shadow-sm text-sm font-medium";
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <AnimatePresence>
            {showUpload && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                ...
                <h2 className="text-2xl font-black text-white mb-6">Upload Unity Asset</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div onClick={() => document.getElementById("pkg-input")?.click()}
                    ...
                    <p className="font-black text-sm text-white">{file ? file.name : "Click to upload .unitypackage or .zip"}</p>
                  ...
                  <div onClick={() => document.getElementById("thumb-input")?.click()}
                    ...
                    <p className="relative z-10 font-black text-sm text-white">{thumbPreview ? "Change thumbnail" : "Upload thumbnail"}</p>
                  ...
                  <div className="col-span-2">
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-[#0A0A0F] border border-gray-200 cursor-pointer" onClick={() => setIsPaid(!isPaid)}>
                      ...
                      <span className="font-bold text-white">Monetize this asset (Razorpay)</span>
                    </div>
                    {isPaid && (
                ...
                <div className="flex gap-4 justify-end mt-8 border-t border-gray-100 pt-6">
                  <button onClick={() => setShowUpload(false)} className="px-6 py-3 rounded-xl border border-gray-200 text-gray-700 font-bold hover:bg-[#0A0A0F] transition">Cancel</button>
          ...
          <div className="mb-8 flex flex-col sm:flex-row gap-4 bg-white/80 p-4 rounded-2xl border border-indigo-100 shadow-sm">
            <input placeholder="Search assets or tags..." value={search} onChange={(e) => setSearch(e.target.value)}
              className="flex-1 bg-[#0A0A0F] border border-gray-200 rounded-xl px-4 py-3 text-sm font-medium text-white placeholder-gray-400 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition"/>
              ...
              {[...Array(6)].map((_, i) => (
                <div key={i} className="rounded-3xl border border-gray-200 bg-white shadow-sm overflow-hidden animate-pulse">
                  ...
                  <div className="p-4 flex flex-col gap-3">
                    ...
                    <div className="h-3 bg-[#0A0A0F] rounded-full w-1/2"/>
                  </div>
                </div>
              ))}
            </div>
              ...
              {visible.map((asset, i) => (
                <motion.div key={asset.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                  ...
                  <div className="p-5 flex flex-col gap-3 flex-1">
                    <div>
                      <p className="font-black text-white text-base leading-tight mb-1">{asset.title}</p>
