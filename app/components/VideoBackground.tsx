...
export default function VideoBackground({
  ...
  const initParticles = useCallback(
    (width: number, height: number) => {
      ...
      for (let i = 0; i < count; i++) {
        const isViolet = Math.random() > 0.3;
        particles.push({
          x: Math.random() * width,
          ...
          color: isViolet ? color : "rgba(255,255,255,0.8)",
  ...
  return (
    ...
    >
      {/* Gradient mesh layer */}
      {(variant === "mesh" || variant === "particles") && (
        <div
          className="absolute inset-0 opacity-[0.12] animate-conic-spin"
          style={{
            background: `conic-gradient(from var(--angle), ${color}, #0A0814, ${color}99, #0A0814, ${color})`,
          }}
        />
      )}
