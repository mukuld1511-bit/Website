...
async function generateRoadmap(params: {
  ageGroup: string; goal: string; level: string; style: string; interests: string;
}): Promise<GeneratedRoadmap> {
  const prompt = `You are an expert XR (AR/VR) education advisor on a platform called SYNTHÉ.
Generate a personalised learning roadmap for someone with these details:
- Age group: ${params.ageGroup}
- Goal: ${params.goal}
- Current level: ${params.level}
- Learning style: ${params.style}
- Specific interests: ${params.interests || "general XR development"}

Return ONLY a valid JSON object with this exact structure (no markdown, no extra text):
{
  "title": "short compelling roadmap title (max 8 words)",
  "summary": "2-sentence personalised intro explaining why this path is right for them",
  "totalDuration": "e.g. 3–4 months",
  "phases": [
    {
      "phase": 1,
      "title": "phase title",
      "duration": "e.g. 2 weeks",
      "description": "1 sentence what they will achieve",
      "topics": ["topic1", "topic2", "topic3"],
      "tools": ["tool1", "tool2"],
      "milestone": "what they can build/show at the end of this phase"
    }
  ],
  "nextStep": "one clear action they should take today to start"
}
Rules:
- 4 to 6 phases depending on the goal
- Keep language age-appropriate (simpler for under-14, more technical for developers)
- tools should be real free tools (Unity, A-Frame, Blender, WebXR API, Babylon.js, etc.)
- milestone must be something concrete and motivating
- Do NOT include any text outside the JSON`;
  ...
  return JSON.parse(raw.replace(/```json|```/g, "").trim()) as GeneratedRoadmap;
}
...
export default function RoadmapPage() {
  ...
  const handleGenerate = async () => {
    ...
    setStep("loading");
    setError("");
    const messages = ["Analysing your profile...", "Mapping the XR ecosystem...", "Selecting tools for you...", "Building your path..."];
    let i = 0;
    const interval = setInterval(() => { i = (i + 1) % messages.length; setLoadingText(messages[i]); }, 1200);
    try {
      const result = await generateRoadmap({ ageGroup, goal, level, style, interests });
  ...
  const reset = () => { setStep("form"); setRoadmap(null); setAgeGroup(""); setGoal(""); setLevel(""); setStyle(""); setInterests(""); };
            ...
            <motion.div key="form" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
              ...
              <div className="mb-10">
                ...
                <p className="text-gray-500 mt-3 text-base max-w-md">
                  Answer 4 questions. Gemini AI builds a step-by-step roadmap made exactly for you.
                </p>
              </div>
              ...
              <div className="space-y-10">
                    ...
                    {GOALS.map((g) => (
                      <button key={g.id} onClick={() => setGoal(g.id)}
                        className={`relative p-4 rounded-2xl border-2 text-left transition-all overflow-hidden ${
                          goal === g.id ? "border-transparent" : "bg-white border-gray-200 hover:border-gray-300"
                        }`}>
                        {goal === g.id && (
                          <div className={`absolute inset-0 bg-gradient-to-br ${g.color} opacity-10`} />
                        )}
                        {goal === g.id && (
                          <div className={`absolute inset-0 border-2 rounded-2xl bg-gradient-to-br ${g.color} opacity-0`}
                            style={{ border: "2px solid", borderImage: "none" }} />
                        )}
                        <div className={`absolute inset-0 rounded-2xl border-2 transition-all ${
                          goal === g.id ? `border-violet-500` : "border-transparent"
                        }`} />
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-2xl font-semibold text-sm transition-all ${
                    canGenerate
                      ? "bg-violet-600 hover:bg-violet-700 text-white shadow-lg shadow-violet-200 hover:shadow-violet-300"
          ...
          {step === "loading" && (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm mt-2">Gemini is crafting your personal XR path</p>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
              ...
              <div className="bg-gray-900 rounded-3xl p-8 relative overflow-hidden">
                ...
                <div className="relative">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-xs font-medium bg-violet-500/20 text-violet-300 border border-violet-500/30 px-3 py-1 rounded-full">
                      AI Generated
                    </span>
                            ...
                            >
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-0.5">
                                  ...
                                  <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${accent.chip}`}>
                                    {phase.duration}
                                  </span>
                                </div>
                                  ...
                                  <div className="px-5 pb-5 border-t border-gray-100 pt-4 grid sm:grid-cols-3 gap-5">
                                      ...
                                      <ul className="space-y-2">
                                        {phase.topics.map((t, i) => (
                                          <li key={i} className="flex items-start gap-2 text-sm text-gray-700">
                                            <span className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${accent.bar}`} />
                                            {t}
                                          </li>
                                        ))}
                                      </ul>
                                    ...
                                    <div>
                                      ...
                                      <div className="flex flex-wrap gap-1.5">
                                        {phase.tools.map((t, i) => (
                                          <span key={i} className="text-xs bg-gray-100 text-gray-700 px-2.5 py-1.5 rounded-lg font-medium">
                                            {t}
                                          </span>
                                        ))}
                                      </div>
              ...
              <div className="flex gap-3 flex-wrap">
                <a href="/learn" className="flex-1 text-center py-3.5 rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold transition-colors shadow-lg shadow-violet-100">
