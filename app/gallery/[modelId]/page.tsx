...
export default function ModelDetailPage() {
  ...
  const trendBadge = engScore >= 70 ? { label: "🔥 Trending", cls: "bg-amber-100 text-amber-700" } : engScore >= 30 ? { label: "⚡ Popular", cls: "bg-purple-100 text-purple-700" } : null;
  ...
  return (
    <div className="min-h-screen bg-[#F7F6F3] flex flex-col">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 flex-grow w-full">
        ...
        {/* Back */}
        <Link href="/verse" className="inline-flex items-center gap-2 text-gray-400 hover:text-gray-700 text-sm font-semibold mb-6 transition">
        ...
        {isWebXRReady && (
          <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}}
            className="mb-6 flex items-center gap-3 px-5 py-3 rounded-2xl bg-gradient-to-r from-[#5B4BDB]/10 to-violet-50 border border-[#5B4BDB]/20">
            ...
            <div className="flex-1">
              <span className="text-sm font-black text-[#5B4BDB]">WebXR Ready</span>
              <span className="text-sm text-gray-500 ml-2">— View this model in your real environment, no app needed</span>
            </div>
            <span className="text-xs font-bold text-[#5B4BDB]/60 hidden sm:block">Scroll down to launch AR →</span>
          </motion.div>
        )}
        ...
        <div className="grid md:grid-cols-5 gap-8">
          ...
          <div className="md:col-span-3 space-y-4">
            ...
            {isWebXRReady && isDownloadable && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-black uppercase tracking-widest text-[#5B4BDB]">WebXR</span>
                  <div className="flex-1 h-px bg-[#5B4BDB]/20"/>
                  <span className="text-xs text-gray-400 font-medium">Augmented Reality</span>
                </div>
            ...
            {isWebXRReady && !isDownloadable && (
              <div className="rounded-2xl border border-dashed border-[#5B4BDB]/30 bg-[#5B4BDB]/5 p-5 text-center">
                <div className="w-10 h-10 rounded-xl bg-[#5B4BDB]/10 flex items-center justify-center mx-auto mb-3">
                  <svg className="w-5 h-5 text-[#5B4BDB]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                </div>
                <p className="text-sm font-bold text-[#5B4BDB] mb-1">WebXR AR — Purchase to unlock</p>
                <p className="text-xs text-gray-400">Buy this model to place it in your real environment via WebXR</p>
              </div>
            )}
            ...
            {(model.tags ?? []).length > 0 && (
              <div className="flex flex-wrap gap-2">
                {model.tags!.map(tag => (
                  <span key={tag} className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${tag.toLowerCase() === "webxr" ? "bg-[#5B4BDB] text-white border-[#5B4BDB]" : "bg-[#5B4BDB]/10 text-[#5B4BDB] border-[#5B4BDB]/20"}`}>
                    #{tag}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-4">Comments {comments.length > 0 && <span className="text-gray-400 font-normal">({comments.length})</span>}</h3>
              {user ? (
                <form onSubmit={handleComment} className="flex gap-2 mb-6">
                  <input value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder="Add a comment..."
                    className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#5B4BDB] transition-colors"/>
                  <button type="submit" disabled={!commentText.trim()} className="px-4 py-2.5 rounded-xl bg-[#5B4BDB] text-white text-sm font-bold disabled:opacity-50 hover:bg-[#4c3ec7] transition-colors">Post</button>
                </form>
              ) : (
                <Link href="/login"><p className="text-sm text-[#5B4BDB] font-semibold mb-4 hover:underline cursor-pointer">Sign in to comment</p></Link>
              )}
                  ...
                  : comments.map(c => (
                    <div key={c.id} className="flex gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#5B4BDB]/10 flex items-center justify-center flex-shrink-0">
                        {c.authorPhoto ? <img src={c.authorPhoto} className="w-full h-full rounded-full object-cover" alt=""/> : <span className="text-[#5B4BDB] text-xs font-bold">{c.authorName.charAt(0)}</span>}
                      </div>
                      <div><p className="text-sm font-semibold text-gray-900">{c.authorName}</p><p className="text-sm text-gray-600 mt-0.5">{c.text}</p></div>
                    </div>
                  ))
                }
              </div>
          ...
          {/* Right */}
          <motion.div className="md:col-span-2 space-y-5" initial={{opacity:0,x:16}} animate={{opacity:1,x:0}} transition={{duration:0.3}}>
            ...
            <div>
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                <div className="flex flex-col gap-1 flex-shrink-0">
                  <span className={`px-2 py-1 rounded-lg text-xs font-bold uppercase ${model.isPaid?"bg-green-100 text-green-700":"bg-blue-100 text-blue-700"}`}>{model.fileType}</span>
                  {isWebXRReady && <span className="px-2 py-1 rounded-lg text-xs font-bold bg-[#5B4BDB] text-white">WebXR</span>}
                  {trendBadge && <span className={`px-2 py-1 rounded-lg text-xs font-bold ${trendBadge.cls}`}>{trendBadge.label}</span>}
                </div>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-full bg-[#5B4BDB]/10 flex items-center justify-center overflow-hidden">
                  {model.authorPhoto ? <img src={model.authorPhoto} className="w-full h-full object-cover" alt=""/> : <span className="text-[#5B4BDB] text-xs font-bold">{model.authorName?.charAt(0)}</span>}
                </div>
            ...
            {isWebXRReady && (
              <div className="bg-gradient-to-br from-[#5B4BDB]/5 to-violet-50 border border-[#5B4BDB]/20 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-6 h-6 rounded-lg bg-[#5B4BDB] flex items-center justify-center flex-shrink-0">
                    <svg className="w-3.5 h-3.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17H3a2 2 0 01-2-2V5a2 2 0 012-2h16a2 2 0 012 2v10a2 2 0 01-2 2h-2"/></svg>
                  </div>
                  <p className="text-sm font-black text-[#5B4BDB]">WebXR Compatible</p>
                </div>
                <p className="text-xs text-gray-500 leading-relaxed mb-3">This model can be placed in your real environment using your phone's camera — powered by the WebXR Device API.</p>
                <div className="flex flex-wrap gap-1.5">
                  {["Chrome Android","Safari iOS 16+","No app needed","Hit-test placement"].map(f=>(
                    <span key={f} className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#5B4BDB]/10 text-[#5B4BDB] border border-[#5B4BDB]/20">{f}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {isXRBuild && model.platforms && (
              <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm space-y-3">
                ...
                {(model.platforms?.length??0)>0&&(
                  <div><p className="text-gray-500 text-sm mb-2">Platforms</p>
                    <div className="flex flex-wrap gap-1.5">{model.platforms!.map(p=><span key={p} className="px-2 py-1 rounded-lg bg-gray-100 text-gray-700 text-xs font-semibold">{p}</span>)}</div>
                  </div>
                )}
                {model.minimumSpecs&&(
                  <div className="space-y-1.5 pt-1 border-t border-gray-100">
                    {Object.entries(model.minimumSpecs).map(([k,v])=>(
                      <div key={k} className="flex justify-between text-xs"><span className="text-gray-500 capitalize">{k}</span><span className="font-semibold text-gray-700">{v as string}</span></div>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="space-y-3">
              {model.isPaid && !purchased ? (
                <>
                  ...
                  {isWebXRReady && <p className="text-xs text-center text-[#5B4BDB] font-medium">Includes WebXR AR viewer</p>}
              ...
              <button onClick={handleLike} disabled={liked}
                className={`w-full py-3 rounded-xl text-sm font-bold border-b-[3px] transition-all active:translate-y-[1px] flex items-center justify-center gap-2 ${liked?"bg-red-50 text-red-500 border-red-200 cursor-default":"bg-white text-gray-700 border-gray-200 hover:bg-gray-50"}`}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-2.5 shadow-sm">
              {[
                ...
                <div key={label} className="flex justify-between text-sm">
                  ...
                  <span className={`font-semibold ${label==="WebXR"?"text-[#5B4BDB]":"text-gray-900"}`}>{value}</span>
                </div>
              ))}
            </div>
          </motion.div>
